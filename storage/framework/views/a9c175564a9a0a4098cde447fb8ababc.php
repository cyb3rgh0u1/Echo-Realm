  ...
  <div style="display:grid; grid-template-columns: 1.8fr 1fr; gap:6rem; align-items:start">
      ...
      <div style="width:100%; aspect-ratio:16/9; background:rgba(255,255,255,0.01); border:1px solid var(--border); border-radius:12px; display:flex; align-items:center; justify-content:center; position:relative; overflow:hidden; box-shadow: 0 30px 60px rgba(0,0,0,0.5)">
        ...
        <?php else: ?>
            <svg width="200" height="200" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="0.3" style="opacity:0.1">
                <?php echo $icons[$item->type] ?? $icons['game']; ?>
    ...
    <div style="padding-top: 0.5rem">
      <div style="display:flex; gap:0.6rem; margin-bottom:1.25rem">
        <span class="badge badge-<?php echo e($item->rarity); ?>" style="font-size:0.6rem; letter-spacing:0.1em"><?php echo e(strtoupper($item->rarity)); ?></span>
        <span class="badge" style="background:rgba(255,255,255,0.02); border:1px solid var(--border); color:var(--text-dim); font-size:0.6rem; letter-spacing:0.1em"><?php echo e(strtoupper($item->type)); ?></span>
      ...
      <h1 style="font-family:var(--font-display); font-size:2.8rem; line-height:1.05; margin-bottom:1.25rem; color:#fff; letter-spacing:-0.01em"><?php echo e($item->name); ?></h1>
      ...
      <div style="background:rgba(255,255,255,0.01); border-left: 2px solid var(--accent); padding:1.25rem 1.5rem; margin-bottom:2.5rem">
        ...
        <div style="display:grid; grid-template-columns: 1fr; gap: 0.5rem">
            <?php $__currentLoopData = $item->includes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $inc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div style="display:flex; align-items:center; gap:0.6rem; color:var(--text-muted); font-size:0.75rem; margin-bottom:0.2rem">
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0; opacity:0.7"><?php echo $icons['bullet']; ?></svg>
                    <?php echo e($inc); ?>
      ...
      <div style="display:flex; align-items:baseline; gap:1.25rem; margin-bottom:2rem">
        <span style="font-family:var(--font-display); font-size:3rem; font-weight:800; color:#fff; letter-spacing:-0.03em">$<?php echo e(number_format($item->price,2)); ?></span>
        <?php if($item->original_price): ?>
            <span style="color:var(--text-dim); font-size:1.1rem; text-decoration:line-through; opacity:0.5">$<?php echo e(number_format($item->original_price,2)); ?></span>
            <span style="color:var(--red); font-family:var(--font-heading); font-size:0.65rem; font-weight:700; letter-spacing:0.1em">/ -<?php echo e($item->discount_percent); ?>%</span>
        <?php endif; ?>
      </div>
      ...
      <form action="<?php echo e(route('cart.add')); ?>" method="POST">
        ...
        <button type="submit" class="btn btn-gold" style="width:100%; justify-content:center; padding:1.2rem; font-size:0.9rem; gap:12px; border-radius:4px; text-transform:uppercase; letter-spacing:0.15em">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><?php echo $icons['cart']; ?></svg>
            Add to Manifest
        </button>
      </form>

      <div style="margin-top:2rem">
        <a href="<?php echo e(route('shop.index')); ?>" class="back-link">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><?php echo $icons['back']; ?></svg>
            Return to Market
        </a>
      </div>
    </div>
...
<?php echo $__env->make('layouts.client', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /opt/lampp/htdocs/echo-realm/resources/views/client/shop/show.blade.php ENDPATH**/ ?>