<?php $__env->startSection('title', $character->name ?? 'Unknown Echo'); ?>
...
<div class="char-hero" style="background: radial-gradient(circle at 0% 100%, <?php echo e($elColor); ?>08, transparent 60%)">
    <div class="char-hero-left">
        <a href="<?php echo e(route('characters.index')); ?>" style="display:flex; align-items:center; gap:10px; color:var(--text-dim); text-decoration:none; font-family:var(--font-heading); font-size:0.6rem; letter-spacing:0.25em; text-transform:uppercase; margin-bottom:4rem; opacity:0.6">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><?php echo $icons['back']; ?></svg>
            Personnel Roster
        </a>

        <div style="font-family:var(--font-heading); font-size:0.65rem; letter-spacing:0.5em; text-transform:uppercase; color:<?php echo e($elColor); ?>; margin-bottom:1.5rem; opacity:0.9">
            <?php echo e($character->element->name ?? 'Neutral'); ?> // RESONANCE SIGNATURE
        </div>
        ...
        <h1 style="font-family:var(--font-display); font-size:clamp(3.5rem, 8vw, 6rem); font-weight:900; line-height:0.85; color:#fff; margin-bottom:1.5rem; letter-spacing:-0.03em">
            <?php echo e($character->name ?? 'Unknown'); ?>

        </h1>

        <p style="color:var(--text-muted); font-size:1.1rem; font-style:italic; margin-bottom:3rem; opacity:0.8; letter-spacing:0.02em">
            "<?php echo e($character->title ?? 'Unidentified Echo'); ?>"
        </p>

        <div style="display:flex; gap:1rem; margin-bottom:4rem">
            <span class="badge badge-<?php echo e($character->rarity ?? 'common'); ?>"><?php echo e(strtoupper($character->rarity ?? 'Common')); ?></span>
            <span class="badge" style="background:rgba(255,255,255,0.03); border:1px solid var(--border); color:var(--text-dim); font-size:0.6rem"><?php echo e(strtoupper($character->role ?? 'None')); ?></span>
        </div>

        <p style="color:var(--text-muted); font-size:1rem; line-height:1.9; max-width:600px; margin-bottom:5rem; opacity:0.7">
            <?php echo e($character->bio ?? 'No background data available.'); ?>

        </p>

        
        <?php if(isset($shopBundle) && $shopBundle): ?>
        <form action="<?php echo e(route('cart.add')); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="item_id" value="<?php echo e($shopBundle->id); ?>">
            <button type="submit" class="btn btn-gold" style="padding: 1.4rem 3rem; gap:15px; font-size:0.9rem; border-radius:4px; letter-spacing:0.1em">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><?php echo $icons['cart']; ?></svg>
                ACQUIRE MANIFEST — $<?php echo e(number_format($shopBundle->price, 2)); ?>
    ...
    <div class="char-hero-right">
        <div class="char-visual-frame">
            <?php if(isset($character->image) && $character->image): ?>
                <img src="<?php echo e(asset('storage/'.$character->image)); ?>" alt="<?php echo e($character->name); ?>" style="width:100%; height:100%; object-fit:cover; filter: grayscale(20%) contrast(110%);">
            <?php else: ?>
                <svg width="240" height="240" viewBox="0 0 24 24" fill="none" stroke="<?php echo e($elColor); ?>" stroke-width="0.3" style="opacity:0.1"><?php echo $icons['hex']; ?></svg>
            <?php endif; ?>
        </div>
    </div>
</div>
...
<div class="section" style="padding-top: 0">
    <div class="container">
        <div class="tabs">
            ...
            <?php if(isset($character->lore) && $character->lore): ?>
        ...
        <div class="tab-pane active" id="tab-abilities">
            <div class="ability-grid">
                <?php if(isset($character->abilities) && is_array($character->abilities)): ?>
                    <?php $__currentLoopData = $character->abilities; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ability): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php
                            $aType = $ability['type'] ?? 'active';
                            $aColor = $aType === 'ultimate' ? 'var(--gold)' : ($aType === 'passive' ? '#10b981' : $elColor);
                        ?>
                        <div class="ability-card" style="border-left: 3px solid <?php echo e($aColor); ?>">
                            <div class="type-tag" style="color: <?php echo e($aColor); ?>">
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><?php echo $icons['module']; ?></svg>
                                <?php echo e($aType); ?> protocol
                            </div>
                            <div style="font-family:var(--font-heading); font-size:1.2rem; font-weight:700; color:#fff; margin-bottom:0.75rem; letter-spacing:0.02em">
                                <?php echo e($ability['name'] ?? 'Unknown Ability'); ?>

                            </div>
                            <p style="color:var(--text-muted); font-size:0.9rem; line-height:1.7; opacity:0.8; max-width: 900px">
                                <?php echo e($ability['description'] ?? 'No description provided.'); ?>
        ...
        <div class="tab-pane" id="tab-stats">
            <div class="stats-hud">
                <?php if(isset($character->stats) && is_array($character->stats)): ?>
                    <?php $__currentLoopData = ['hp'=>'Integrity','attack'=>'Offense','defense'=>'Mitigation','magic'=>'Resonance','speed'=>'Velocity']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php if(isset($character->stats[$key])): ?>
                            <div class="stat-row">
                                <span class="stat-label"><?php echo e($label); ?></span>
                                <div class="stat-track">
                                    <div class="stat-fill" style="width:<?php echo e($character->stats[$key]); ?>%; color:<?php echo e($elColor); ?>; background:currentColor"></div>
                                </div>
                                <span style="font-family:var(--font-heading); font-size:0.8rem; color:#fff; text-align:right; font-weight:700"><?php echo e($character->stats[$key]); ?></span>
                            </div>
                        <?php endif; ?>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                <?php endif; ?>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.client', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /opt/lampp/htdocs/echo-realm/resources/views/client/characters/show.blade.php ENDPATH**/ ?>