...
<div style="padding:8rem 2rem 6rem;max-width:1100px;margin:0 auto">
  ...
  <div style="text-align:center;padding:6rem 2rem;background:rgba(255,255,255,0.02);border:1px solid var(--border);border-radius:16px">
    <div style="margin-bottom:1.5rem; color:var(--text-dim)">
        <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"><?php echo $icons['empty']; ?></svg>
    </div>
    ...
    <a href="<?php echo e(route('shop.index')); ?>" class="btn btn-outline" style="padding: 0.8rem 2rem">Return to Market</a>
  </div>
  <?php else: ?>
  <div style="display:grid;grid-template-columns:1fr 380px;gap:3rem;align-items:start">
    
    <div>
      <?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ci): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <?php $item=$ci['item']; $qty=$ci['qty']; ?>
      <div style="display:grid;grid-template-columns:100px 1fr auto;gap:1.5rem;align-items:center;padding:1.5rem;background:rgba(255,255,255,0.02);border:1px solid var(--border);border-radius:12px;margin-bottom:1rem">
        ...
        <div style="aspect-ratio:1;background:var(--void);border-radius:8px;display:flex;align-items:center;justify-content:center;border:1px solid rgba(255,255,255,0.05);position:relative;overflow:hidden">
            ...
            <?php else: ?>
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="1.5" style="opacity:0.6"><?php echo $icons[$item->type] ?? $icons['game']; ?></svg>
            <?php endif; ?>
        </div>

        
        <div>
          <div style="font-family:var(--font-heading);font-size:1rem;font-weight:600;color:#fff;margin-bottom:0.2rem"><?php echo e($item->name); ?></div>
          <div style="color:var(--text-dim);font-size:0.7rem;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.8rem"><?php echo e($item->type); ?></div>
          <div style="color:var(--accent);font-family:var(--font-heading);font-size:1.1rem;font-weight:700">$<?php echo e(number_format($item->price,2)); ?></div>
        </div>

        
        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:0.75rem">
          <form action="<?php echo e(route('cart.update')); ?>" method="POST" style="display:flex;align-items:center;gap:0.5rem">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="item_id" value="<?php echo e($item->id); ?>">
            <input type="number" name="qty" value="<?php echo e($qty); ?>" min="1" max="99" 
          ...
          <form action="<?php echo e(route('cart.remove')); ?>" method="POST" style="margin:0">
            ...
            <button type="submit" style="background:none;border:none;color:var(--red);cursor:pointer;display:flex;align-items:center;gap:5px;font-size:0.7rem;text-transform:uppercase;letter-spacing:0.1em;opacity:0.7">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><?php echo $icons['trash']; ?></svg>
                Remove
            </button>
          </form>
        </div>
    ...
    <div style="background:rgba(255,255,255,0.03);border:1px solid var(--border);border-radius:16px;padding:2rem;position:sticky;top:100px">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.75rem;font-size:0.85rem">
        <span style="color:var(--text-muted)"><?php echo e($ci['item']->name); ?> <small style="opacity:0.5;margin-left:4px">×<?php echo e($ci['qty']); ?></small></span>
        <span style="font-family:var(--font-heading);color:#fff">$<?php echo e(number_format($ci['item']->price * $ci['qty'],2)); ?></span>
      </div>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2rem">
        ...
        <span style="font-family:var(--font-display);font-size:1.6rem;font-weight:800;color:var(--accent)">$<?php echo e(number_format($total,2)); ?></span>
      </div>

      <?php if(auth()->guard()->check()): ?>
      <a href="<?php echo e(route('checkout')); ?>" class="btn btn-gold" style="width:100%;justify-content:center;padding:1rem;font-size:0.9rem">Initialize Checkout</a>
      <?php else: ?>
      <a href="<?php echo e(route('login')); ?>" class="btn btn-primary" style="width:100%;justify-content:center;padding:1rem;font-size:0.9rem">Login to Proceed</a>
      <?php endif; ?>

      <a href="<?php echo e(route('shop.index')); ?>" style="display:flex;align-items:center;justify-content:center;gap:8px;margin-top:1.25rem;color:var(--text-dim);font-family:var(--font-heading);font-size:0.65rem;text-decoration:none;letter-spacing:0.15em;text-transform:uppercase">
        Return to Market
      </a>
    </div>
  </div>
...
<?php echo $__env->make('layouts.client', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /opt/lampp/htdocs/echo-realm/resources/views/client/shop/cart.blade.php ENDPATH**/ ?>