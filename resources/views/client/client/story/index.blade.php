...
<div class="section">
  <div class="container" style="max-width: 1000px">
    @forelse($arcs as $arcNum => $chapters)
    @php $firstChapter = $chapters->first(); @endphp
    <div class="arc-section reveal">
      
      <div class="arc-header">
        <div class="arc-number">{{ sprintf('%02d', $arcNum) }}</div>
        <div class="arc-info">
          <h2>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right: 8px; vertical-align: middle; opacity: 0.5">{!! $icons['arc'] !!}</svg>
            Arc {{ $arcNum }}
          </h2>
          <span class="arc-status {{ $firstChapter->status }}">{{ $firstChapter->status }}</span>
        </div>
      ...
      @foreach($chapters as $story)
      <a href="{{ route('story.show', $story->slug) }}" class="chapter-row">
        <div class="chapter-num">{{ sprintf('%02d', $story->chapter_number) }}</div>
        <div>
          <div class="chapter-title">{{ $story->title }}</div>
          <p class="chapter-synopsis">{{ $story->synopsis }}</p>
        </div>
        <div class="read-btn">
          ACCESS
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">{!! $icons['read'] !!}</svg>
        </div>
      </a>
      @endforeach
