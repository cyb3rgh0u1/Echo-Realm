  ...
  <div class="timeline-wrapper">
    ...
    @foreach($events as $event)
      @if($event->era !== $currentEra)
        @php $currentEra = $event->era; @endphp
        <div class="era-label reveal"><span>{{ $currentEra }}</span></div>
      @endif
      ...
      <div class="tl-item reveal {{ $isEven ? 'tl-left' : 'tl-right' }}" style="transition-delay:{{ ($itemIdx % 6) * 0.1 }}s">
        
        <div class="tl-content-wrapper">
            <div class="tl-content">
              <div class="tl-header">
                <svg class="tl-icon" viewBox="0 0 24 24" fill="none" stroke="{{ $event->color }}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    {!! $svgPath !!}
                </svg>
                <span class="tl-year" style="color:{{ $event->color }}">{{ $event->year_in_lore }}</span>
              </div>
        ...
        <div class="tl-dot-col">
          <div class="tl-dot" style="background:{{ $event->color }}; box-shadow: 0 0 15px {{ $event->color }}88;"></div>
        </div>
